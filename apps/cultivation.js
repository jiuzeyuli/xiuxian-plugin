...
export default class Cultivation {
  ...
  async checkCultivation(e) {
    ...
    const nextRealm =
      user.realm < this.realms.length - 1 ? this.realms[user.realm + 1] : null;
  ...
  async breakthrough(e) {
    ...
    if (success) {
      ...
      resultMsg.push(
        ...
        `💎 消耗灵石：${stoneCost}`,
        `❤️ 生命上限提升！`,
        `✨ 下一境界：${this.realms[user.realm + 1]?.name || "飞升"}（需 ${
          nextRealm.exp
        } 修为）`
      );
  ...
  async tribulation(e) {
    ...
    resultMsg.push(
      tribulationDesc[tribulationType.name] || "天地变色，劫难降临！"
    );
    ...
    if (success) {
      ...
      if (user.pills[6] && user.pills[6] > 0) {
        ...
        user.exp = 0;
        resultMsg.push(
          `💥 渡劫失败，境界跌落至 ${this.realms[user.realm].name}`
        );
      }
  ...
  async seclusion(e) {
    ...
    if (now - user.lastSeclusion < this.cdTimes.seclusion) {
      const remaining = Math.ceil(
        (this.cdTimes.seclusion - (now - user.lastSeclusion)) / 3600000
      );
  ...
  async adventure(e) {
    ...
    if (now - user.lastAdventure < this.cdTimes.adventure) {
      const remaining = Math.ceil(
        (this.cdTimes.adventure - (now - user.lastAdventure)) / 60000
      );
  ...
  async challengeDungeon(e) {
    ...
    if (now - user.lastDungeon < this.cdTimes.dungeon) {
      const remaining = Math.ceil(
        (this.cdTimes.dungeon - (now - user.lastDungeon)) / 60000
      );
    ...
    if (user.realm < dungeon.minRealm) {
      return e.reply(
        `❌ 境界不足！需要 ${this.realms[dungeon.minRealm].name} 才能挑战 ${
          dungeon.name
        }`
      );
    }
    ...
    this.addLog(
      userId,
      `挑战秘境 ${dungeon.name} ${success ? "成功" : "失败"}`
    );
  ...
  async handleDualCultivationAgreement(e) {
    ...
    this.addLog(
      userId,
      `与${this.getUserName(requesterId)}双修，获得修为${expGain}`
    );
    this.addLog(
      requesterId,
      `与${this.getUserName(userId)}双修，获得修为${expGain}`
    );

    // 通知双方
    await e.reply(
      [
        `💞 ${e.sender.card || e.sender.nickname} 与 ${this.getUserName(
          requesterId
        )} 开始双修...`,
        `✨ 双方修为 +${expGain}`,
        `🍀 双方气运 +5`,
    ...
    // 通知请求者
    await this.app.bot.sendPrivateMsg(
      requesterId,
      [
  ...
  async sectInfo(e) {
    ...
    if (sect.members.length > 0) {
      ...
      topMembers.forEach((member) => {
        msg.push(
          `  ${member.title} ${this.getUserName(member.id)} - ${
            this.realms[member.realm].name
          }`
        );
      });
    }
  ...
  async joinSect(e) {
    ...
    const sectId = Object.keys(sects).find((id) => sects[id].name === sectName);
  ...
  async cultivationLog(e) {
    ...
    const logList = user.logs
      .slice()
      .reverse()
      .slice(0, 5)
      .map(
        (log) => `[${new Date(log.time).toLocaleTimeString()}] ${log.content}`
      )
      .join("\n");
  ...
  async viewBackpack(e) {
    ...
    // 显示背包容量
    const itemCount = Object.values(backpack.items).reduce(
      (sum, count) => sum + count,
      0
    );
  ...
  async buyItem(e) {
    ...
    // 检查背包容量
    const itemCount = Object.values(user.backpack.items).reduce(
      (sum, count) => sum + count,
      0
    );
    if (itemCount + quantity > user.backpack.capacity) {
      return e.reply(
        `❌ 背包空间不足！需要 ${quantity} 格空间，剩余 ${
          user.backpack.capacity - itemCount
        } 格`
      );
    }
  ...
  async viewPillsSystem(e) {
    ...
    const pillList = this.pills
      .map((pill) => {
        ...
        return `${pill.id}. ${pill.name} ★${pill.quality} - ${
          pill.desc
        }\n  效果: ${
          pill.effect > 0
            ? `+${pill.effect}修为`
  ...
  async viewSpiritRootsSystem(e) {
    ...
    const spiritRootList = this.spiritRoots
      .map((root, index) => {
        const isCurrent = index === user.spiritRoot;
        return `${index}. ${root.name}${
          isCurrent ? " (当前)" : ""
        }\n  修炼效率: ×${root.expRate} | 炼丹加成: ×${
          root.alchemy
        } | 突破加成: ×${root.breakthrough} | 气运加成: +${root.luck} | 描述: ${
          root.desc
        }`;
      })
      .join("\n\n");
  ...
  async viewTribulationSystem(e) {
    const tribulationList = this.tribulationTypes
      .map(
        (t) =>
          `⚡ ${t.name} (${t.level})\n  描述: ${t.desc}\n  伤害: ${t.damage}%生命值`
      )
  ...
  async viewSectsList(e) {
    ...
    const sectList = Object.values(sects)
      .map(
        (sect) =>
  ...
  async viewRealmsSystem(e) {
    ...
    const realmList = this.realms
      .map((realm, index) => {
        ...
        return `${index}. ${realm.name}${
          isCurrent ? " (当前境界)" : ""
        }\n  修为要求: ${realm.exp > 0 ? realm.exp : "圆满"} | 描述: ${
          realm.desc
        }`;
      })
      .join("\n\n");
